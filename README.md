# Cartoon Clock / サボる時計 (v0.2.6)

※ v0.2.6 は「登場アニメ 5パターン」のみを実装した検証版です（tier/爆発/解散アニメは未実装）。

### v0.2.6 の変更点（主に①-a）
- 伸び表現はバッファ全体のスケール変形ではなく、**粒子の移動**で「横に伸びる→縦に伸びる→反動→到達直後ポヨン」を表現
- 数字に戻るのは **アニメの最後**（それまでは液体のまま）
...
## v0.1.17
- 見られていない状態へ切り替わる瞬間の「パキッと太くなる」違和感を解消（見られ/見られないの見た目パラメータを短時間でスムーズに補間）
