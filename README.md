...
挙動確認のため、左上の **🧪** からテストパネルを開けます（`T` キーでも開閉）。

- ENTER（上下/左右/ランダム）
- EXIT（赤→爆発 / 透明で消える / 距離感ズーム追従）
- IDLE（サボり） / DISPLAY（即・時刻）
- 自動EXIT（3秒見続け）ON/OFF
- ショートカット：`1/2/3`=ENTER、`4/5/6`=EXIT、`I`=IDLE、`D`=DISPLAY、`S`=Seen切替
...
## v0.2.0
- ENTER/EXIT を仕様に合わせて作り直し。
  - ENTER: いったん「もこっ」と集まってから、バウンドしつつ数字へ収束（上下 / 左右 / 多方向ランダム）。
  - EXIT: 時刻表示になってから **3秒** 見られ続けると、ランダムで解散。
    - 赤→爆発（コロン高速）
    - 透明→消える
    - 距離感ズームアウト追従（顔位置の反対方向へ奥に逃げる）
